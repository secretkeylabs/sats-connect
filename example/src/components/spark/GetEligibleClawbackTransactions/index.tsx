...
import { request, SparkGetEligibleClawbackTransactionsResult } from 'sats-connect';
import { Button, Card } from '../../../App.styles';
import { useGlobalState } from '../../GlobalStateProvider/use-global-state';

export const GetEligibleClawbackTransactions = () => {
  const { sparkAddressInfo } = useGlobalState();
  const [transactions, setTransactions] = useState<SparkGetEligibleClawbackTransactionsResult>();

  const onClick = useCallback(() => {
    (async () => {
      const response = await request('spark_flashnet_getEligibleClawbackTransactions', null);
  ...
  return (
    <Card>
      ...
      <Button onClick={onClick}>Call spark_flashnet_getEligibleClawbackTransactions</Button>
    </Card>
  );
};
