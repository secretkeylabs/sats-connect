...
### Build a PSBT input from address metadata

Wallets can return the scripts needed to construct an input. Decode them before passing the input to `@scure/btc-signer`:

```ts
const decoded = decodeAddressPsbtData(address);

tx.addInput({
  txid,
  index,
  witnessUtxo: { script: decoded.scriptPubKey, amount },
